...
from ReNetutility import MyReNetReg50000
...
loader_trainAE, num_trainAE, loader_testAE, num_testAE, loader_valAE, num_valAE = generate_AE(mat, batch_size)

resnet = MyReNetReg50000(input_channel = 1, layers = [1,1,1], regression_out = 1)
#count = count_parameters(resnet)
#print(count)
resnet.apply(init_weight)

resnet = resnet.cuda(3)
#summary(resnet, (1, 5000))
criterion = nn.MSELoss().cuda(3)
...
for epoch in range(num_epochs):
    ...
    for i, (samples, labels) in enumerate(loader_trainAE):
        #print(samples.size())
        #print(labels)
        samplesV = Variable(samples.cuda(3))
        labels = labels.squeeze()
        labelsV = Variable(labels.cuda(3))
...
savemat("/home/zhuochengjiang/project-seong/result_AE_50000.mat", {'Pred':y_pred, 'True':y_true})
#torch.save(resnet.state_dict(), '/home/zhuochengjiang/project-seong/reset-AE5000-regression2.pt')
